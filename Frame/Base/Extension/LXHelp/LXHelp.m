...
+ (NSArray *)timeForTheRecentDate
{
    ...
    NSArray *today = [LXHelp getYearAndMonthAndDayFromTimeString:curTime];
    [dateArr addObject:today];
    
    
    //明天
    NSString *timeStamp = [LXHelp timeStringIntoTimeStamp:curTime];
    NSInteger seconds = 24*60*60 + [timeStamp integerValue];
    timeStamp = [NSString stringWithFormat:@"%ld",(long)seconds];
    curTime = [LXHelp timeStampIntoTimeString:timeStamp];
    
    NSArray *tomorrow = [LXHelp getYearAndMonthAndDayFromTimeString:curTime];
...
+ (BOOL)judgePhoneTypeIsCanFingerprint
{
    ...
    if ([LXHelp getUserPhoneModelNumber].length >= 7)
    {
        phoneModelNum = [[LXHelp getUserPhoneModelNumber] substringWithRange:range];
    }
