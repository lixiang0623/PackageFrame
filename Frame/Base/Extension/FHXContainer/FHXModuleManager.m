...
@interface FHXModuleManager ()
@property(nonatomic, strong) NSMutableArray *modules;
@property(nonatomic, strong) NSMutableSet *selectorByEvent;
@property(nonatomic, strong) NSMutableDictionary<NSString *, NSMutableArray<id<FHXModuleProtocol>> *> *modulesByEvent;
@end

@implementation FHXModuleManager

+ (instancetype)shareInstance {
    static FHXModuleManager *manager;
...
- (void)registerModule:(Class)moduleClass {
    ...
    if ([moduleClass conformsToProtocol:@protocol(FHXModuleProtocol)]) {
        //实例化
        id<FHXModuleProtocol> moduleInstance = [[moduleClass alloc] init];
        ...
        MagicLog(@">>>FHXContainerError:[%@]没有实现FHXModuleProtocol协议", moduleName);
    }
}

- (void)registerEventWithModuleInstance:(id<FHXModuleProtocol>)moduleInstance {
...
- (void)registerEvent:(NSString *)event withModuleInstance:(id<FHXModuleProtocol>)moduleInstance {
    ...
    if (![modulesOfEvent containsObject:moduleInstance]) {
        ...
        [modulesOfEvent sortUsingComparator:^NSComparisonResult(id<FHXModuleProtocol> module1, id<FHXModuleProtocol> module2) {
...
- (void)unregisterModuleWithModule:(Class)moduleClass {
    ...
    [self.modulesByEvent enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSMutableArray<id<FHXModuleProtocol>> * _Nonnull obj, BOOL * _Nonnull stop) {
        __block NSInteger index = -1;
        
        [obj enumerateObjectsUsingBlock:^(id<FHXModuleProtocol>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:moduleClass]) {
                index = idx;
                *stop = YES;
            }
...
- (void)triggerModuleEvent:(NSString *)event
                withTarget:(id<FHXModuleProtocol>)target
...
- (void)triggerModuleInstallEventWithTarget:(id<FHXModuleProtocol>)target withCustomParam:(NSDictionary *)customParam {
    //根据优先级排序
    [self.modules sortUsingComparator:^NSComparisonResult(id<FHXModuleProtocol> module1, id<FHXModuleProtocol> module2) {
        NSInteger module1Priority = 0;
        NSInteger module2Priority = 0;
        if ([module1 respondsToSelector:@selector(priority)]) {
            module1Priority = [module1 priority];
    ...
    NSArray<id<FHXModuleProtocol>> *moduleInstances;
    if (target) {
...
- (void)triggerModuleInitEventWithTarget:(id<FHXModuleProtocol>)target withCustomParam:(NSDictionary *)customParam {
    NSArray<id<FHXModuleProtocol>> *moduleInstances;
    if (target) {
        moduleInstances = @[target];
    }else{
    ...
    FHXContext *context = [FHXContext shareInstance].copy;
    context.customEvent = kModInitEvent;
    context.customParam = customParam;
    
    [moduleInstances enumerateObjectsUsingBlock:^(id<FHXModuleProtocol>  _Nonnull moduleInstance, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)triggerModuleUninstallEventWithTarget:(id<FHXModuleProtocol>)target withCustomParam:(NSDictionary *)customParam {
    NSArray<id<FHXModuleProtocol>> *moduleInstances;
    if (target) {
        moduleInstances = @[target];
    }else{
    ...
    [moduleInstances enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id<FHXModuleProtocol>  _Nonnull moduleInstance, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSMutableDictionary<NSString *,NSMutableArray<id<FHXModuleProtocol>> *> *)modulesByEvent {
    if (!_modulesByEvent) {
...
- (NSMutableSet *)selectorByEvent {
    if (!_selectorByEvent) {
        ...
        struct objc_method_description *methodDescriptions = protocol_copyMethodDescriptionList(@protocol(FHXModuleProtocol), NO, YES, &numberOfMethods);
