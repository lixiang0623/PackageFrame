...
@property (nonatomic, copy  ) NSString *imageGifName;//gif图片名称
...
- (NSString *)fhx_imageName{
  if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(imageNameForEmptyDataSet:)]) {
    NSString *name = [self.emptyDataSetSource imageNameForEmptyDataSet:self];
    return name;
  }
  return nil;
}

- (NSString *)fhx_imageGifName{
  if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(imageGifNameForEmptyDataSet:)]) {
    NSString *name = [self.emptyDataSetSource imageGifNameForEmptyDataSet:self];
    return name;
  }
  return nil;
}
        ...
        if (customView) {
            ...
            if (image) {
                if ([image respondsToSelector:@selector(imageWithRenderingMode:)]) {
                    ...
                    view.imageGifName = [self fhx_imageGifName];//gif图片名称
                    view.imageName = [self fhx_imageName];//获取图片名称
                }
                else {
...
- (void)setupConstraints
{
  ...
  //添加contentView的水平方向左右贴边
    ...
    if (_customView) {
        ...
        CGFloat width = CGRectGetWidth(self.frame) ? : CGRectGetWidth([UIScreen mainScreen].bounds);//414
        CGFloat padding = roundf(width/16.0);//26
        CGFloat verticalSpace = self.verticalSpace ? : 11.0; // 默认11 pts
        
        NSMutableArray *subviewStrings = [NSMutableArray array];//0
        NSMutableDictionary *views = [NSMutableDictionary dictionary];//0
        ...
        if (_imageView.superview) {
            ...
            //_imageView CenterX 跟contentViewy一样

          if ([self.imageName isEqualToString:self.imageGifName]) {
            //根据项目自定义自定义过
            NSLayoutConstraint *height = [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:0 constant:60];
            
            NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:0 constant:60];
            
            [self.contentView addConstraints
             :@[[self.contentView equallyRelatedConstraintWithView:_imageView attribute:NSLayoutAttributeCenterX],height,width]];
          }else{
            //原生约束
        ...
        // 为垂直约束构建动态字符串格式，并在每个元素之间添加边距。 默认值为11点。
