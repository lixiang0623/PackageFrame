...
#import "FHXNewPagedFlowView.h"

@interface FHXNewPagedFlowView ()
...
@implementation FHXNewPagedFlowView
...
- (void)initialize{
    ...
    subviewClassName = @"FHXIndexBannerSubiew";
...
- (void)autoNextPage {
    ...
    switch (self.orientation) {
        case FHXNewPagedFlowViewOrientationHorizontal:{
            
            [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:YES];
            break;
        }
        case FHXNewPagedFlowViewOrientationVertical:{
...
- (void)queueReusableCell:(FHXIndexBannerSubiew *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    FHXIndexBannerSubiew *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case FHXNewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                FHXIndexBannerSubiew *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    FHXIndexBannerSubiew *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, FHXIndexBannerSubiew *cell) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark FHXNewPagedFlowView API
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[FHXIndexBannerSubiew class]]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case FHXNewPagedFlowViewOrientationHorizontal://横向
...
- (FHXIndexBannerSubiew *)dequeueReusableCell{
    FHXIndexBannerSubiew *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(FHXIndexBannerSubiew *)cell {
