...
#define FHX_ScreenWidth                   [UIScreen mainScreen].bounds.size.width
#define FHX_ScreenHeight                  [UIScreen mainScreen].bounds.size.height

static const CGFloat kDefaultCellHeight = 54;
static const CGFloat kFrontCancelTableViewCellHeight = 10;

@interface FHXResourceSheetView ()<UITableViewDelegate,
...
@implementation FHXResourceSheetView
...
- (void)makeSubviewsLayout {
    ...
    self.frame = CGRectMake(0, FHX_ScreenHeight - height, FHX_ScreenWidth, height);
    self.tableView.frame = CGRectMake(0, 0, FHX_ScreenWidth, height);
}
...
- (void)tapHandle:(UIGestureRecognizer *)tap {
    if ([self.delegate respondsToSelector:@selector(FHXResourceSheetView:seletedMode:)]) {
        [self.delegate FHXResourceSheetView:self seletedMode:ResourceModeNone];
    }
    [self hide];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSArray *arr = self.resourceFroms[section];
    return arr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(cell == nil){
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, FHX_ScreenWidth, kDefaultCellHeight)];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    UIView *header = [[UIView alloc] initWithFrame:CGRectMake(0, 0, FHX_ScreenWidth, height)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(FHXResourceSheetView:seletedMode:)]) {
        [self.delegate FHXResourceSheetView:self seletedMode:mode];
    }    
    [self hide];
}
